...
SRC_URI=""
EGIT_REPO_URI="https://github.com/unrealircd/unrealircd.git"
...
pkg_preinst() {
	...
	# Usage: _unrealircd_dir_permissions <user> <group> <mode> <dir>[, <dir>â¦]
	#
	...
	# permissionsâbut only if we are replacing an unrealircd ebuild at
		...
		while dir=${1} && shift; do
			if [[ ! -d "${EROOT}${dir}" ]]; then
				...
				eend ${?}
					...
					# If 3.2.10 â¤ ${REPLACING_VERSIONS}, then we update
